 ...
 * QuestForm
 * ---------
 * Shared two-step quest editor used by both the create flow and the edit
 * flow for unpublished (draft) quests. In create mode it submits via
 * `createQuest`; in edit mode it submits via `updateQuest` against an
 * existing draft.
 */
...
export default function QuestForm({ mode, initial }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    quest_type: (initial?.quest_type ?? 'solo') as QuestType,
    difficulty: (initial?.difficulty ?? 'standard') as DifficultyPreset,
    estimated_duration: initial?.estimated_duration ?? '1-2 hours',
    min_participants: initial?.min_participants ?? 1,
    max_participants: initial?.max_participants ?? 1,
    everloop_overlay: initial?.everloop_overlay ?? true,
    narrator_style: initial?.narrator_style ?? 'atmospheric',
    narrator_pacing: initial?.narrator_pacing ?? 'moderate',
    branching_narrative: initial?.branching_narrative ?? true,
    tags: initial?.tags ?? '',
    regions: initial?.regions ?? [],
    location_ids: initial?.location_ids ?? [],
    publish_now: initial?.publish_now ?? (mode === 'create'),
  })

  const [graph, setGraph] = useState<QuestFlowGraph>(() => initial?.graph ?? defaultGraph())
  ...
  async function handleSubmit() {
    ...
    if (mode === 'create') {
      ...
      // In edit mode we only allow editing while the quest is a draft. The
      // "publish_now" toggle promotes a draft → available on save.
      const result = await updateQuest(initial.id, {
        ...payload,
        status: form.publish_now ? 'available' : 'draft',
      })
      if (result.success && result.quest) {
  ...
  const headerTitle = mode === 'create' ? 'Create a' : 'Edit'
  const submitLabelIdle = mode === 'create' ? '✦ Create Quest' : '✦ Save Changes'
  const submitLabelLoading = mode === 'create' ? 'Weaving the Loop...' : 'Saving...'
  ...
  const StepHeader = (
    <div className="mb-8">
      ...
      <h1 className="text-4xl font-serif mb-2">
        <span className="text-parchment">{headerTitle}</span>{' '}
      ...
      <p className="text-parchment-muted mb-6">
        {mode === 'create'
          ...
          : 'Refine your draft. Changes are private until you publish — when ready, toggle “Publish now” to make this quest joinable from the Quest Portal.'}
      </p>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          <div className="flex items-center gap-3">
            <Link
              href={mode === 'edit' && initial ? `/quests/${initial.slug}` : '/quests'}
              className="text-sm text-parchment-muted hover:text-parchment transition-colors"
            >
              Cancel
            </Link>
            <Button onClick={handleSubmit} disabled={loading} className="btn-fantasy">
              {loading ? submitLabelLoading : submitLabelIdle}
            </Button>
          </div>
        </div>
      </div>
