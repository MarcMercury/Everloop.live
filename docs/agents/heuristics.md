...
| **Live-Play mechanics** (Bell Sequence, Sound Meter, Redirect Die, NPC reset, escalation failures, NPC presentations, scene callbacks, hidden mechanics) live in JSONB — quest-level at `quests.metadata.liveplay` (typed by `LivePlayConfig`) and scene-level at `quest_scenes.metadata.live_play` (typed by `SceneLivePlay`). Do NOT add new columns for these — keep them in metadata so they can evolve per quest. | 2026-06 |
| Scene callbacks reference scenes by their `metadata.slug`, not UUID, so they survive a delete-then-insert reseed. Always set `metadata.slug` on a scene when you intend to reference it from another scene. | 2026-06 |
| When syncing a quest to an updated source PDF, prefer full DELETE + INSERT of `quest_scenes` over UPSERT-by-title — old scenes that no longer match get orphaned otherwise. Only safe if no live sessions reference the quest yet. | 2026-06 |
| `QuestSceneInsert` / `QuestSceneUpdate` accept a `metadata?: Json` field — use this to thread arbitrary scene metadata (e.g. `live_play`) through `createScene` / `updateScene` without changing server actions. | 2026-06 |
| **Session Media** (mic, camera, audio, sound library) lives in `lib/media/` as browser hooks (`useAudioPlayer`, `useMicSoundMeter`, `useCameraCapture`). Scene-level config persists at `quest_scenes.metadata.media` (typed by `SceneMediaConfig`). Soundboard builder UI uses textarea format `Label \| url \| icon` per line — parser builds structured `sfx_buttons[]`. | 2026-06 |
| Session image captures upload through `/api/sessions/captures` (DM-only, service role) into the `session-captures` bucket. Never give the browser direct write access to the bucket; storage policy is public-read only. | 2026-06 |
| Freesound search route returns 503 `{ error, hint }` when `FREESOUND_API_KEY` missing — graceful degradation. Catalog ambience/SFX in `lib/media/sound-libraries.ts` point at `/public/audio/library/*.mp3`; CC0/Pixabay files are user drop-ins, not committed to the repo. | 2026-06 |
| Camera/mic require a user gesture — never auto-start either on page load. SessionMediaPanel surfaces an "auto-arm" banner when scene wants the sound meter armed but the user hasn't clicked Mic yet. | 2026-06 |
